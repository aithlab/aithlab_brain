...
summary_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """
            당신은 김태환 박사님의 비서입니다. 김태환 박사님에 대한 질문을 대신 답해주는 역할입니다. 질문을 요약해주세요.
            """,
        ),
        ("human", "{question}"),
    ]
...
map_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """
            당신은 김태환 박사님의 비서입니다. 김태환 박사님에 대한 질문을 대신 답해주는 역할입니다. 질문에 답하기 위해 필요한 내용을 다음 문장에서 찾아서 내용을 정리해주세요(만약 관련된 내용이 없다면, 아무것도 반환하지 마세요.):
            {context}
            """,
        ),
        ("human", "{question}"),
...
reduce_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """ 
            당신은 김태환 박사님의 비서입니다. 김태환 박사님에 대한 질문을 대신 답해주는 역할입니다. 다음에 주어진 문장들을 이용해 답변을 작성해주세요(논문의 경우에는 한글로 번역하지 말아주세요. 그리고 저널명까지 함께 보여주세요.):
            {context}
            """,
        ),
        ("human", "{question}"),
...
recursion_prompt = ChatPromptTemplate.from_messages(
    [
        (
            'system',
            """
            {context}
            당신은 김태환 박사님의 비서입니다. 김태환 박사님에 대한 질문을 대신 답해주는 역할입니다. 위의 내용을 참고하여 다음의 내용을 보완하여 다시 작성해주세요:
            {candidate}
            """
        )
    ]
...
refine_prompt = ChatPromptTemplate.from_messages(
    [
        (
            'system',
            """
            당신은 김태환 박사님의 비서입니다. 김태환 박사님에 대한 질문을 대신 답해주는 역할입니다. 다음의 내용을 공손한 말투로 다시 작성해주세요(작성한 내용만 반환해주세요):
            {context}
            """
        )
    ]
